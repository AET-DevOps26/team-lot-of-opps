...
class TestEmbedEndpoint:
    ...
    def test_embed_update_returns_ok(self):
        requests.post(
            f"{BASE_URL}/embed",
            json={"invoice_id": TEST_INVOICE_ID, "text": "original text"},
            timeout=10,
        )
        r = requests.put(
            f"{BASE_URL}/embed",
            json={"invoice_id": TEST_INVOICE_ID, "text": "updated text about laptop purchase"},
            timeout=10,
        )
        assert r.status_code == 200
        assert r.json() == {"status": "ok"}
    ...
    def test_delete_returns_ok(self):
        requests.post(
            f"{BASE_URL}/embed",
            json={"invoice_id": TEST_INVOICE_ID, "text": "some invoice text"},
            timeout=10,
        )
...
class TestRagChat:
    def test_chat_returns_answer(self):
        ...
        r = requests.post(
            f"{BASE_URL}/api/chat", json={"question": "What work equipment did I buy?"}, timeout=120
        )
    ...
    def test_chat_references_embedded_content(self):
        ...
        r = requests.post(
            f"{BASE_URL}/api/chat", json={"question": "What hotels did I stay in?"}, timeout=120
        )
    ...
    def test_chat_without_user_id(self):
        r = requests.post(
            f"{BASE_URL}/api/chat", json={"question": "What are my expenses?"}, timeout=120
        )
        assert r.status_code == 200
        assert "answer" in r.json()
    ...
    def test_chat_with_null_user_id(self):
        r = requests.post(
            f"{BASE_URL}/api/chat",
            json={"question": "What are my expenses?", "user_id": None},
            timeout=120,
        )
        assert r.status_code == 200
    ...
    def test_delete_removes_from_search(self):
        ...
        r_before = requests.post(
            f"{BASE_URL}/api/chat",
            json={"question": "What is the xylophone purple banana invoice?"},
            timeout=120,
        )
        assert (
            "999" in r_before.json()["answer"] or "xylophone" in r_before.json()["answer"].lower()
        )
