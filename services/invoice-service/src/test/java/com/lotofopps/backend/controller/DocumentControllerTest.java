...
class DocumentControllerTest {
    ...
    @MockitoBean private DocumentStorageService documentStorageService;

    @MockitoBean private DocumentRepository documentRepository;

    @MockitoBean private InvoiceRepository invoiceRepository;

    @MockitoBean private ExtractionService extractionService;

    @MockitoBean private LlmChatEmbeddingService embeddingService;

    @Test
    void uploadDocumentReturnsUploadResponse() throws Exception {
        Document document =
                new Document("invoice.pdf", "application/pdf", 13L, "/uploads/some-uuid.pdf");
    ...
    void uploadUnsupportedFileTypeReturnsBadRequest() throws Exception {
        MockMultipartFile file =
                new MockMultipartFile("file", "document.txt", "text/plain", "some text".getBytes());
    ...
    void uploadOversizedFileReturnsBadRequest() throws Exception {
        ...
        MockMultipartFile file =
                new MockMultipartFile("file", "big.pdf", "application/pdf", largeContent);
    ...
    void uploadReturns500WhenExtractionFails() throws Exception {
        ...
        when(documentStorageService.store(any(), any())).thenReturn(document);
        when(extractionService.extractAndStore(any()))
    ...
    void listDocumentsReturnsDocumentResponseFields() throws Exception {
        Document document =
                new Document("report.pdf", "application/pdf", 42L, "/uploads/report.pdf");
        when(documentRepository.findByUserId(any())).thenReturn(List.of(document));

        mockMvc.perform(get("/api/v1/documents").header("X-User-Sub", "test-user"))
                .andExpect(status().isOk())
