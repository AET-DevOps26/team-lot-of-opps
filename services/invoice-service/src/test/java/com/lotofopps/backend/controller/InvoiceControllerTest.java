...
class InvoiceControllerTest {
    ...
    void listInvoicesReturnsInvoiceResponseFields() throws Exception {
        ...
        when(invoiceRepository.findByUserIdAndStatus(
                        anyString(), any(InvoiceStatus.class), any(Pageable.class)))
                .thenReturn(new PageImpl<>(List.of(invoice)));

        mockMvc.perform(get("/api/v1/invoices").header("X-User-Sub", "test-user"))
    ...
    void listInvoicesAcceptsYearQueryParam() throws Exception {
        when(invoiceRepository.findByUserIdAndStatusAndInvoiceDateYear(
                        anyString(), any(InvoiceStatus.class), anyInt(), any(Pageable.class)))
    ...
    void deleteInvoiceReturns204WhenOwned() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/invoices/1").header("X-User-Sub", "test-user"))
    ...
    void acceptInvoiceSetsAcceptedStatusAndEmbeds() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/invoices/1/accept").header("X-User-Sub", "test-user"))
                .andExpect(status().isOk())
    ...
    void deleteInvoiceReturns404WhenNotFound() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/invoices/99").header("X-User-Sub", "test-user"))
