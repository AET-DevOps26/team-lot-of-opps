...
function InvoiceFormModal({ mode, initial, onSave, onClose }: InvoiceFormModalProps) {
  const t = useT()
...
export default function Invoices() {
  ...
  function handleModalSave(saved: InvoiceResponse) {
    setInvoices((prev) => {
    ...
    <div className="space-y-lg">
      {modalMode && (
        <InvoiceFormModal
          ...
          onClose={() => { setModalMode(null); setEditingInvoice(null) }}
        />
      )}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <button
            type="button"
            onClick={() => { setEditingInvoice(null); setModalMode('create') }}
            className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-primary text-on-primary font-label-caps text-label-caps uppercase hover:bg-primary/90 transition-colors shadow-sm"
          >
                ...
                >
                  ...
                  <td className="py-4 px-6 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <button
                        onClick={() => { setEditingInvoice(inv); setModalMode('edit') }}
                        className="text-primary hover:bg-primary/10 p-2 rounded-full transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100"
                        title={t('invoices.table.edit')}
                      >
                        <Icon name="edit" />
                      </button>
                      <button
